...
SELECT * FROM subjects OVER BY location;
...
JOIN stock s ON s.isbnn =e.isbn
...
SELECT books.title,edition.isbn,publishers.name,stock.retail
FROM editions
JOIN books ON editions.book
...
SELECT editions.book_id,books.title,count(e.book_id) AS editions
